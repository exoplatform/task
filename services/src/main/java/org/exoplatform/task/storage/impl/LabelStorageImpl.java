...
public class LabelStorageImpl implements LabelStorage {
    ...
    public ListAccess<LabelDto> findLabelsByUser(String username) {
        return (ListAccess<LabelDto>) labelToDto((Label) daoHandler.getLabelHandler().findLabelsByUser(username));
    }

    @Override
    public ListAccess<LabelDto> findLabelsByTask(long taskId, String username) throws EntityNotFoundException {
        return (ListAccess<LabelDto>) labelToDto((Label) daoHandler.getLabelHandler().findLabelsByTask(taskId, username));
    }
    ...
    public LabelDto labelToDto(Label label) {
        ...
        labelDto.setParent(label.getParent());
        return labelDto;
    }
}
