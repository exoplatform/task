...
public class CommentDAOImpl extends GenericDAOImpl<Comment, Long> implements CommentDAO {
  ...
  public long count(Task task) {
    Long count = entityManager.createNamedQuery("Comment.countCommentOfTask", Long.class)
  ...
  public List<Comment> findCommentsOfTask(Task task, int start, int limit) {
    Query query = entityManager.createNamedQuery("Comment.findCommentsOfTask", Comment.class);
